...
class MRNetDataset(Dataset):
    ...
    def __init__(self, root_dir, task, train=True, transform=None, require_all_views=False):
        """
        Args:
            root_dir (str): Directory with processed data
        ...
        # Look for processed data in the correct location: src/processed/data
        self.data_dir = os.path.join(root_dir, 'src', 'processed', 'data')
        
        # Check if the directory exists, and try alternative paths if not
        if not os.path.exists(self.data_dir):
            print(f"WARNING: Processed data not found at {self.data_dir}")
            # Try other possible locations
            alt_paths = [
                os.path.join(root_dir, 'processed_data'),  # Original expected path
                os.path.join(root_dir, 'src', 'processed_data'),
                os.path.join(root_dir, 'processed'),
                os.path.join(root_dir, 'src', 'processed')
            ]
            
            for path in alt_paths:
                if os.path.exists(path):
                    self.data_dir = path
                    print(f"Found processed data at: {self.data_dir}")
                    break
        else:
        ...
        if not os.path.exists(self.data_dir):
            print(f"ERROR: Could not find processed data directory. Please ensure the normalized data exists.")
